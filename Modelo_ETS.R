...
for (sexo in unique(tabua$SEXO)) {
  for (idade in unique(tabua$IDADE)) {
    ...
    ets_previsoes <- rbind(ets_previsoes, data.frame(
      ...
      Modelo = "ETS"  # Identificador do modelo
    ))
  }
...
# 2. Distribuição dos métodos ETS
print("\nDistribuição dos Métodos ETS:")
print(table(ets_metricas$ETS_Metodo))

# 3. Métricas agregadas por sexo
print("\nMétricas Globais ETS por Sexo:")
print(ets_globais)

# 4. Exemplo gráfico comparativo
idade_exemplo <- 30
sexo_exemplo <- "Homens"

dados_plot_ets <- ets_previsoes %>% 
  filter(IDADE == idade_exemplo, SEXO == sexo_exemplo)

ggplot(dados_plot_ets, aes(x = ANO)) +
  geom_line(aes(y = Observado, color = "Observado"), linewidth = 1) +
  geom_line(aes(y = Previsao, color = "Previsão ETS"), linewidth = 1) +
  geom_ribbon(aes(ymin = X95., ymax = X95..1), alpha = 0.2, fill = "blue") +
  labs(title = paste("Previsão ETS para", sexo_exemplo, "de", idade_exemplo, "anos"),
       y = "Taxa de Mortalidade (nMx)", x = "Ano") +
  scale_color_manual(values = c("Observado" = "black", "Previsão ETS" = "green4")) +
  theme_minimal()

# 4. Gráfico de todas as previsões
ggplot(ets_previsoes, aes(x = ANO, y = Previsao, color = SEXO)) +
  geom_line() +
...
write.csv(ets_metricas, "C:/Users/cleod/OneDrive/Documentos/Documentos/Estudos/Ciências Atuariais/TCC/ets_metricas.csv", row.names = FALSE)
write.csv(ets_previsoes, "C:/Users/cleod/OneDrive/Documentos/Documentos/Estudos/Ciências Atuariais/TCC/ets_previsoes.csv", row.names = FALSE)
write.csv(ets_globais, "C:/Users/cleod/OneDrive/Documentos/Documentos/Estudos/Ciências Atuariais/TCC/ets_globais.csv", row.names = FALSE)
